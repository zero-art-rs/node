...
pub async fn update_art(
    ...
    let secretes = (0..2).map(|i| ScalarField::rand(&mut rng)).collect();
    let g = ARTG::generator();
    let (t, _) = ART::<ARTG>::new_art_from_secrets(&secretes, &g);
    // info!("art: {}", serde_json::to_string(&t).unwrap());
    let mut user_agent = ARTUserAgent::new(t, secretes[0].clone());
    let (rk, changes) = user_agent.update_key().unwrap();


    info!("changes: {}", serde_json::to_string(&changes).unwrap());
