...
template addZeroOptimisedBabyjub(){
    signal input in1[2];
    signal input in2[2];
    signal output out[2];
    ...
    isZero.in <== adder.out[0];

    component switcherX = Switcher();
    switcherX.bool <== isZero.out;
    switcherX.in[0] <== in1[0] + in2[0];
    switcherX.in[1] <== adder.out[0];
    component switcherY = Switcher();
    switcherY.bool <== isZero.out;
    switcherY.in[0] <== in1[1] + in2[1];
    switcherY.in[1] <== adder.out[1];

    out[0] <== switcherX.out[1];
    out[1] <== switcherY.out[1];
}
//---------------------------------------------------------------------------------------------------------------------------------------
...
template BabyjubjubBase8Multiplication(){
    ...
    for (var i = 0; i < 254; i++){
        adders[i] = addZeroOptimisedBabyjub();
        if (i == 0){
            adders[i].in1 <== [0,0];
